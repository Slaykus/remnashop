...
inline-invite =
    ...
    .description = Надёжный VPN без ограничений. Первые дни — бесплатно.
    .message =
        🌧️ Rain VPN

        Стабильный и безопасный доступ к интернету без ограничений.
        Сервис подходит для личного использования и работы.

        Новым пользователям предоставляется бесплатный доступ для тестирования.
    .start = Попробовать бесплатно

inline-proxy =
    .title = Прокси для Telegram
    .description = Подключиться к Telegram через прокси-сервер Rain
    .message =
        📡 Прокси для Telegram

        Telegram заблокирован? Подключитесь через прокси-сервер Rain VPN.
        Работает без VPN-приложений — достаточно одного нажатия.
    .connect = 🔗 Подключить прокси
...
frg-user =
    ...
    { $referral_tier ->
    [1] • <b>Статус</b>: 💧 Капля
    [2] • <b>Статус</b>: ☁️ Облако
    [3] • <b>Статус</b>: ⛈️ Шторм
    [4] • <b>Статус</b>: 🌧️ Дождь
    *[0] { empty }
    }
