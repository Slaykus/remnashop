...
ntf-gateway =
    ...
    .gift-issued =
        <e id="5433658017299184705">🎁</e> <b>Подарочная подписка оплачена</b>

        Код для получателя: <code>{ $code }</code>

        Передайте его тому, кому дарите. Код одноразовый и действует { $days } дней — после этого сгорает.
...
# Ad link (our system) notifications
ntf-ad =
    .bonus-received =
...
ntf-partner =
    .payout-confirmed =
        <e id="5427009714745517609">✅</e> <b>Партнёр подтвердил получение</b>

        { $name } подтвердил, что деньги дошли.
    .earning =
        <e id="5433758580765754681">💰</e> <b>Начисление { $amount } ₽</b>

        Приведённый вами человек оплатил подписку на { $payment } ₽.
        Ваша доля по ставке { $rate }% — <b>{ $amount } ₽</b>.

        Станет доступной к выплате { $available }.

    .available =
        <e id="5427009714745517609">✅</e> <b>{ $amount } ₽ можно вывести</b>

        Срок удержания прошёл. Запросить выплату — в кабинете.

    .paid =
        <e id="5433758580765754681">💸</e> <b>Выплата { $amount } ₽ отправлена</b>

        Если деньги не пришли — напишите, разберёмся.

    .terms-changed =
        <e id="5424858262950111605">🔄</e> <b>Условия изменились</b>

        <blockquote>
        • <b>Ставка</b>: { $rate }%
        • <b>Удержание</b>: { $hold } дн.
        • <b>Минимальная выплата</b>: { $min_payout } ₽
        </blockquote>

        Уже начисленное пересчитано не будет: каждое начисление хранит ту
        ставку, по которой посчитано.
    .payout-requested =
        <e id="5433758580765754681">💰</e> <b>Партнёр просит выплату</b>
        ...
        Оформить можно в разделе рекламы, карточка партнёра.
