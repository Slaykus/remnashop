# Это единственный файл переводов в вашем volume — он имеет наивысший приоритет.
# Встроенные переводы бота лежат в образе (assets.default/) и всегда актуальны;
# обновления бота их не трогают, а этот файл остается вашим.

# Этот файл служит для двух целей:
#
# 1. Переопределение встроенных переводов.
#    Чтобы изменить любой стандартный ключ — скопируйте его сюда со своим текстом.
#    Значение отсюда перекроет встроенное (assets.default/), остальные ключи берутся из бота.
#
# 2. Собственные переводы, которых нет в стандартном наборе.
#    Например: дополнительные кнопки меню, названия планов и т.п.
#    Используйте ключ вместо текста (например, в названии плана).
#    Чтобы не пересекаться со встроенными ключами, давайте им префикс custom-.

# Ключи должны быть уникальными. Подробная документация по переводам: assets/README.md.

# Максимальная длина перевода:
# Для использования в кнопках — 32 символа.
# Для использования в сообщениях — 1024 символа.

# ============================================================
# КАСТОМНЫЕ КЛЮЧИ МЕНЮ
# ============================================================

custom-menu-link1 = 1️⃣ Первая кнопка
custom-menu-link2 = 2️⃣ Вторая кнопка

# ============================================================
# СИСТЕМНЫЕ МЕТКИ
# ============================================================

# Метка squad для импортированных пользователей
IMPORTED = 🔄 Импортирован

# ============================================================
# НАЗВАНИЯ И ОПИСАНИЯ ПЛАНОВ Rain VPN
# ============================================================

Free = 🌧️ Free | Бесплатный
Solo = 👤 Solo | 2 устройства
Duo = 👥 Duo | 4 устройства
Family = 👨‍👩‍👧 Family | 6 устройств
Team = 🏢 Team | 10 устройств

plan-desc-free = Бесплатный доступ, чтобы спокойно проверить сервис в реальных условиях.
plan-desc-solo = Безлимитный VPN для личного использования. Подключай свои устройства и пользуйся без ограничений.
plan-desc-duo = Расширенный тариф для двух пользователей или нескольких устройств.
plan-desc-family = Оптимально для всей семьи.
plan-desc-team = Для команды или проекта. Один тариф — много устройств.

# ============================================================
# КНОПКИ ПОЛЬЗОВАТЕЛЬСКОГО МЕНЮ — брендирование Rain VPN
#
# Эмодзи ниже — обычные Unicode: они работают сразу и служат запасным
# вариантом для клиентов без поддержки кастом-эмодзи.
#
# После загрузки премиум-пака в Telegram заполните brand/emoji/ids.json и
# выполните `node brand/emoji/apply-ids.mjs` — скрипт подставит теги
# <e id="..."> в ключи внутри маркеров rain-emoji. Маркеры не удалять:
# скрипт правит только участок между ними.
#
# Атрибуты можно переопределять точечно — недостающие берутся из встроенных
# переводов образа, целиком копировать блок не нужно.
# ============================================================

# >>> rain-emoji

btn-menu =
    .trial = <e id="5434043784549673084">🌧️</e> ПОПРОБОВАТЬ БЕСПЛАТНО
    .trial-paid = <e id="5434043784549673084">🌧️</e> ПОПРОБОВАТЬ ЗА { $trial_price }
    .connect = <e id="5431350621011746205">⚡</e> Подключиться
    .connect-reserve = <e id="5433768631764822220">🔗</e> Подключиться (резерв)
    .devices = <e id="5433608163196706522">📱</e> Мои устройства
    .subscription = <e id="5431887891355703774">💎</e> Подписка
    .invite = <e id="5434067728992347183">👥</e> Пригласить друга
    .support = <e id="5431834255804113657">💬</e> Поддержка
    .web-cabinet = <e id="5431837348180565162">🌐</e> Личный кабинет
    .dashboard = <e id="5431452321542350670">🎛</e> Панель управления
    .proxy = 👀 Прокси для друга

    .connect-not-available =
    ⚠️ { $status ->
    [LIMITED] ПРЕВЫШЕН ЛИМИТ ТРАФИКА
    [EXPIRED] ПОДПИСКА ИСТЕКЛА
    *[OTHER] ПОДПИСКА НЕ АКТИВНА
    } ⚠️

btn-invite =
    .about = <e id="5431426830911448115">❓</e> Подробнее о награде
    .copy = <e id="5433724552515460070">📋</e> Скопировать ссылку
    .send = <e id="5433790699306787710">📩</e> Пригласить
    .qr = <e id="5431763569232355197">🔳</e> QR-код
    .withdraw-points = <e id="5431658449907789518">⭐</e> Обменять баллы
    .reset-referral = <e id="5431450139698962745">🔄</e> Сбросить реф. ссылку

btn-devices =
    .delete-all = <e id="5433693255088776864">🗑</e> Удалить все устройства
    .reissue = <e id="5431450139698962745">🔄</e> Перевыпустить подписку
    .confirm-delete = <e id="5433838236004820271">✅</e> Да, удалить
    .confirm-reissue = <e id="5433838236004820271">✅</e> Да, сбросить
    .cancel-reissue = <e id="5433773811495378237">❌</e> Нет

btn-subscription =
    .plan = <e id="5431887363074728345">🗂</e> Перейти к оформлению подписки
    .new = <e id="5433718990532814817">💰</e> Купить подписку
    .renew = <e id="5433858061573858779">🔁</e> Продлить
    .change = <e id="5433956609598464803">🔀</e> Изменить
    .promocode = <e id="5431503878329774916">🎟</e> Активировать промокод
    .promocode-confirm = <e id="5433838236004820271">✅</e> Подтвердить
    .pay = <e id="5433625699548179370">💳</e> Оплатить
    .get = <e id="5431744001361352063">🎁</e> Получить бесплатно
    .back-plans = <e id="5434137616700186409">⬅️</e> Назад к выбору плана
    .back-duration = <e id="5434137616700186409">⬅️</e> Изменить длительность
    .back-payment-method = <e id="5434137616700186409">⬅️</e> Изменить способ оплаты
    .connect = <e id="5431350621011746205">⚡</e> Подключиться

btn-back =
    .general = <e id="5434137616700186409">⬅️</e> Назад
    .menu = <e id="5433682723828966023">🏠</e> Главное меню
    .menu-return = <e id="5433682723828966023">🏠</e> Вернуться в главное меню

btn-common =
    .cancel = <e id="5433773811495378237">❌</e> Отмена
    .notification-close = <e id="5433773811495378237">❌</e> Закрыть

# Кастомные кнопки пользовательского меню
btn-how-connect = <e id="5433622474027740592">🧭</e> Как подключиться?
btn-proxy = 👀 Поделиться Mtproxy
btn-proxy-share = <e id="5433790699306787710">📩</e> Отправить в Telegram
btn-proxy-copy = <e id="5433724552515460070">📋</e> Скопировать ссылку

# <<< rain-emoji

# ============================================================
# КНОПКИ АДМИН-ПАНЕЛИ: node-quota (наши кастомные атрибуты)
# Upstream btn-user не содержит этих атрибутов.
#
# Атрибут .transaction здесь намеренно отсутствует: он дословно повторял
# встроенный, но ссылался на { gateway-type } из другого бандла — из custom.ftl
# такая ссылка не резолвится, и кнопка падала с FormatError.
# ============================================================

btn-user =
    .discount = 💸 Скидка
    .discount-personal = 👤 Персональная скидка
    .discount-purchase = 🎟 На следующую покупку
    .points = 💎 Баллы
    .statistics = 📊 Статистика
    .referrals = 👪 Рефералы
    .message = 📩 Сообщение
    .role = 👮‍♂️ Роль
    .transactions = 🧾 Транзакции
    .give-access = 🔑 Доступ к планам
    .current-subscription = 💳 Текущая подписка
    .subscription-traffic-limit = 🌐 Лимит трафика
    .subscription-device-limit = 📱 Лимит устройств
    .subscription-expire-time = ⏳ Время истечения
    .subscription-squads = 🔗 Сквады
    .subscription-traffic-reset = 🔄 Сбросить трафик
    .subscription-devices = 🗒️ Список устройств
    .node-quota = 🔴 Обход белых списков
    .node-quota-reset = 🔄 Сбросить счётчик
    .node-quota-restrict = 🚫 Ограничить доступ
    .node-quota-unrestrict = ✅ Снять ограничение
    .node-quota-test-notify = 📨 Тест уведомления
    .subscription-url = 📋 Скопировать ссылку
    .subscription-delete = ❌ Удалить
    .subscription-reissue = ♻️ Перевыпустить
    .message-preview = 👀 Предпросмотр
    .message-confirm = ✅ Отправить
    .referral-reset = 🔄 Сбросить реф. ссылку
    .sync = 🌀 Синхронизировать
    .sync-remnawave = 🌊 Использовать данные Remnawave
    .sync-remnashop = 🛍 Использовать данные Remnashop
    .give-subscription = 🎁 Выдать подписку
    .subscription-internal-squads = ⏺️ Внутренние сквады
    .subscription-external-squads = ⏹️ Внешний сквад

    .allowed-plan-choice = { $selected ->
    [1] 🔘
    *[0] ⚪
    } { $plan_name }

    .subscription-active-toggle = { $is_active ->
    [1] 🔴 Выключить
    *[0] 🟢 Включить
    }

    .trial-toggle = { $is_trial_available ->
    [1] 🧪 Пробник: доступен
    *[0] 🧪 Пробник: не доступен
    }

    .block = { $is_blocked ->
    [1] 🔓 Разблокировать
    *[0] 🔒 Заблокировать
    }

# ============================================================
# СООБЩЕНИЯ: ПЕРЕОПРЕДЕЛЕНИЯ СТАНДАРТНЫХ КЛЮЧЕЙ
# ============================================================

# frg-subscription: явно определён здесь для надёжности (также есть в utils.ftl из assets.default)
frg-subscription =
    <blockquote>
    • <b>Лимит трафика</b>: { $traffic_limit }
    • <b>Лимит устройств</b>: { $device_limit }
    • <b>Осталось</b>: { $expire_time }
    </blockquote>

# msg-subscription-main переопределён: добавлен блок node-quota
# (upstream версия — просто однострочный текст "<b><e id="5431887891355703774">💳</e> Подписка</b>")
msg-subscription-main =
    <b><e id="5431887891355703774">💳</e> Подписка</b>

    { $has_current_subscription ->
    [1] { frg-subscription }
    *[0] { "" }
    }

    { $node_quota_enabled ->
    [1]
    <b><e id="5434006396859357687">🔴</e> Обход белых списков:</b>
    <blockquote>
    { $node_quota_is_restricted ->
    [1] • <e id="5431537503628730769">🚫</e> Доступ ограничен до 1-го числа следующего месяца
    *[0]
    • <b>Использовано</b>: { $node_quota_used_gb } / { $node_quota_limit_gb } ГБ
    • <b>Свободно</b>: { $node_quota_free_gb } ГБ
    }
    </blockquote>
    *[0] { "" }
    }

# msg-menu-invite переопределён: Rain VPN брендинг + таблица тиров реферальной программы
msg-menu-invite =
    <b><e id="5434067728992347183">🤝</e> Реферальная программа</b>

    Приглашайте пользователей по вашей уникальной ссылке и получайте { $reward_type ->
        [POINTS] <b>баллы</b> — обменивайте на дни доступа или подписку
        [EXTRA_DAYS] <b>бесплатные дни</b> к вашей текущей подписке
        *[OTHER] { $reward_type }
    }.

    <b><e id="5431409921625205825">📊</e> Ваша статистика:</b>
    <blockquote>
    • Приглашено пользователей: { $referrals }
    • Оплат по вашей ссылке: { $payments }
    { $reward_type ->
    [POINTS] • Баллов на счёте: { $points }
    *[EXTRA_DAYS] { "" }
    }
    </blockquote>

    <b><e id="5431658449907789518">🏆</e> Статус и персональная скидка:</b>
    <blockquote>
    За каждого приглашённого, кто оформит подписку, вы получаете постоянную скидку на все тарифы:

    • <e id="5431362105754296012">☁️</e> Облако — <b>5%</b> (1 чел.)
    • <e id="5431433165988209445">🌩️</e> Туча — <b>10%</b> (3 чел.)
    • <e id="5433946872907606413">🌧️</e> Дождь — <b>15%</b> (5 чел.)
    • <e id="5431693264912690049">⛈️</e> Шторм — <b>25%</b> (10 чел.)

    { $referral_tier ->
    [1] Ваш статус: <e id="5431362105754296012">☁️</e> Облако — скидка <b>{ $personal_discount }%</b>
    [2] Ваш статус: <e id="5431433165988209445">🌩️</e> Туча — скидка <b>{ $personal_discount }%</b>
    [3] Ваш статус: <e id="5433946872907606413">🌧️</e> Дождь — скидка <b>{ $personal_discount }%</b>
    [4] Ваш статус: <e id="5431693264912690049">⛈️</e> Шторм — скидка <b>{ $personal_discount }%</b>
    *[0] Ваш статус: пока не получен.
    }
    </blockquote>

# msg-menu-invite-about переопределён: Rain VPN брендинг
msg-menu-invite-about =
    <b><e id="5431673473703389665">💡</e> Условия программы</b>

    <b>Условие начисления:</b>
    <blockquote>
    { $accrual_strategy ->
    [ON_FIRST_PAYMENT] Вознаграждение начисляется однократно — за первую оплату подписки приглашённым пользователем.
    [ON_EACH_PAYMENT] Вознаграждение начисляется за каждую оплату или продление подписки приглашённым пользователем.
    *[OTHER] { $accrual_strategy }
    }
    </blockquote>

    <b>Размер вознаграждения:</b>
    <blockquote>
    { $max_level ->
    [1] За приглашённых: { $reward_level_1 }
    *[MORE]
    { $identical_reward ->
    [0]
    • Уровень 1 (ваши приглашённые): { $reward_level_1 }
    • Уровень 2 (приглашённые ваших друзей): { $reward_level_2 }
    *[1]
    За все уровни: { $reward_level_1 }
    }
    }

    { $reward_strategy_type ->
    [AMOUNT] { $reward_type ->
        [POINTS] { "" }
        [EXTRA_DAYS] <i>Дополнительные дни начисляются к вашей текущей подписке.</i>
        *[OTHER] { $reward_type }
    }
    [PERCENT] { $reward_type ->
        [POINTS] <i>Процент рассчитывается от стоимости подписки приглашённого.</i>
        [EXTRA_DAYS] <i>Процент рассчитывается от длительности подписки приглашённого.</i>
        *[OTHER] { $reward_type }
    }
    *[OTHER] { $reward_strategy_type }
    }
    </blockquote>

# msg-menu-proxy: страница "Поделиться Mtproxy" (кастомная, нет в upstream)
msg-menu-proxy =
    📡 <b>Прокси для Telegram</b>

    Если у друга заблокирован Telegram — эта ссылка поможет. Работает без VPN-приложений, достаточно просто перейти по ней.

    Отправь любым удобным способом:

# ============================================================
# СООБЩЕНИЯ: КАСТОМНЫЕ КЛЮЧИ (НЕ СУЩЕСТВУЮТ В UPSTREAM)
# ============================================================

# Окно администратора: просмотр квоты обхода белых списков конкретного пользователя
msg-user-node-quota =
    <b><e id="5434006396859357687">🔴</e> Обход белых списков</b>

    <blockquote>
    • <b>Нода</b>: 🇷🇺<e id="5434006396859357687">🔴</e> Обход белых списков
    • <b>Лимит</b>: { $node_quota_limit_gb } ГБ / мес
    • <b>Использовано</b>: { $node_quota_used_gb } ГБ ({ $node_quota_pct }%)
    • <b>Свободно</b>: { $node_quota_free_gb } ГБ
    • <b>Период</b>: с { $period_start }
    • <b>Статус</b>: { $is_restricted ->
    [1] <e id="5431537503628730769">🚫</e> Ограничен
    *[0] <e id="5433838236004820271">✅</e> Активен
    }
    { $restricted_at ->
    [0] { "" }
    *[HAS] • <b>Ограничен с</b>: { $restricted_at }
    }
    </blockquote>

# Диалог платного сброса трафика на обходе белых списков (пользователь)
msg-subscription-traffic-reset-confirm =
    <b><e id="5434006396859357687">🔴</e> Сброс трафика</b>

    Сбросьте счётчик использованного трафика на сервере обхода белых списков.

    <b>Использовано:</b> { $tr_used_gb } из { $tr_limit_gb } ГБ ({ $tr_used_pct }%)
    <b>Свободно:</b> { $tr_free_gb } ГБ

    { $tr_show_warning ->
    [1]
    <e id="5431713747611722012">⚠️</e> <b>У вас ещё остался неизрасходованный трафик.</b> Сброс счётчика выгоден только при исчерпании лимита. Платёж будет списан в любом случае.

    *[0] { "" }
    }<b>Стоимость:</b> { $price } ₽

    После успешной оплаты счётчик будет немедленно обнулён, и вы снова получите доступ к полному лимиту.

    { $tr_url_has ->
    [1] <e id="5433838236004820271">✅</e> Платёж создан. Нажмите кнопку ниже для оплаты.
    *[0] Выберите способ оплаты:
    }

# ============================================================
# УВЕДОМЛЕНИЯ: КАСТОМНЫЕ КЛЮЧИ (НЕ СУЩЕСТВУЮТ В UPSTREAM)
# ============================================================

# Уведомления системы квоты трафика на обходе белых списков
ntf-node-quota =
    .reset-purchased = <e id="5433838236004820271">✅</e> <i>Счётчик трафика на обходе белых списков успешно сброшен!</i>

    .reset-purchased-system =
        <e id="5431887891355703774">💳</e> <b>Платный сброс трафика</b>

        Пользователь: <code>{ $telegram_id }</code> ({ $name }, @{ $username })
        Сумма: <b>{ $price } ₽</b>
        Сброшено трафика: <b>{ $used_gb } ГБ</b>
        Был ограничен: { $was_restricted ->
            [1] да
            *[0] нет
        }

    .reset-by-admin-system =
        🛠 <b>Сброс трафика администратором</b>

        Администратор: <code>{ $admin_telegram_id }</code> ({ $admin_name })
        Пользователь: <code>{ $target_telegram_id }</code> ({ $target_name }, @{ $target_username })
        Сброшено трафика: <b>{ $used_gb } ГБ</b>
        Был ограничен: { $was_restricted ->
            [1] да
            *[0] нет
        }

    .warn =
        <e id="5431713747611722012">⚠️</e> Вы использовали <b>{ $used_gb } ГБ</b> из <b>{ $limit_gb } ГБ</b> месячного лимита на сервере обхода белых списков.

        При достижении 100% доступ к этому серверу будет ограничен до конца месяца.

    .limited =
        <e id="5431537503628730769">🚫</e> Ваш месячный лимит на сервере обхода белых списков (<b>{ $limit_gb } ГБ</b>) исчерпан.

        Доступ к серверу будет восстановлен 1-го числа следующего месяца.{ $reset_price ->
            [0] { "" }
            *[other]  Вы также можете <b>сбросить счётчик досрочно</b> за { $reset_price } ₽ в разделе «Подписка».
        }

# ============================================================
# РЕКЛАМНЫЕ ССЫЛКИ: КНОПКИ (ОТСУТСТВУЮТ В UPSTREAM 0.8.0)
# ============================================================

btn-advertising =
    .create = ➕ Создать ссылку
    .toggle-active = { $is_active ->
    [1] 🔴 Деактивировать
    *[0] 🟢 Активировать
    }
    .edit-name = ✏️ Название
    .bonus-points = 🎯 Баллы: { $bonus_points }
    .bonus-days = 📅 Дней: { $bonus_days }
    .bonus-discount = 💸 Скидка навсегда: { $bonus_discount_pct }%
    .bonus-discount-once = 🎟 Скидка на первую покупку: { $bonus_discount_once_pct }%
    .edit-code = 🔗 Код: { $code }
    .link-owner = { $is_partner_link ->
    [true] 🤝 Партнёр: { $owner_name }
    *[false] 🤝 Привязать к партнёру
    }
    .delete = 🗑 Удалить
    .delete-confirm = ✅ Подтвердить удаление
    .promo = ✉️ Promo-сообщение
    .promo-edit-text = ✏️ Изменить текст
    .promo-edit-markdown = { $promo_is_markdown ->
    [1] 📐 Разметка (включена)
    *[0] 📐 Написать разметкой
    }
    .promo-photo = { $promo_has_photo ->
    [1] 🖼 Изменить вложение
    *[0] 🖼 Добавить вложение
    }
    .promo-remove-photo = 🗑 Удалить вложение
    .promo-add-button = ➕ Добавить кнопку
    .promo-use-bot-url = 🤖 Ссылка на бота
    .promo-use-site-url = 🌐 Ссылка на сайт
    .promo-preview = 👁 Отправить мне превью
    .promo-send = 🚀 Отправить ботом
    .promo-send-confirm = ✅ Отправить
    .promo-publish = 📣 Опубликовать через себя
    .promo-style-default = ⚪ Обычная
    .promo-style-primary = 🔵 Синяя
    .promo-style-success = 🟢 Зелёная
    .promo-style-danger = 🔴 Красная
    .analytics = 📊 Аналитика
    .analytics-period-7 = { $is_active_7 ->
    [1] ● 7 дней
    *[0] ○ 7 дней
    }
    .analytics-period-30 = { $is_active_30 ->
    [1] ● 30 дней
    *[0] ○ 30 дней
    }
    .analytics-period-0 = { $is_active_0 ->
    [1] ● Всё время
    *[0] ○ Всё время
    }
    .analytics-trend = 📈 Тренд кликов
    .analytics-funnel = 📊 Воронка
    .comparison = 📊 Сравнить
    .comparison-sort-revenue = { $is_sort_revenue ->
    [1] ● Выручка
    *[0] ○ Выручка
    }
    .comparison-sort-conversion = { $is_sort_conversion ->
    [1] ● Конверсия
    *[0] ○ Конверсия
    }
    .comparison-sort-clicks = { $is_sort_clicks ->
    [1] ● Клики
    *[0] ○ Клики
    }
    .comparison-chart = 📊 График

    .partners = <e id="5433614043006903194">🤝</e> Партнёры
    .partner-add = <e id="5431449001532594346">➕</e> Добавить партнёра
    .partners-comparison = <e id="5431449001532594346">📊</e> Сравнение
    .partner-rate = <e id="5433758580765754681">💰</e> Ставка
    .partner-hold = <e id="5424900830821172096">⏳</e> Удержание
    .partner-min = <e id="5433758580765754681">💳</e> Минимум
    .partner-toggle = <e id="5424858262950111605">🔄</e> Вкл/выкл
    .partner-pay = <e id="5427009714745517609">✅</e> Выплатить
    .partner-links = <e id="5431492767249342148">🔗</e> Ссылки
    .partner-bonus = <e id="5433614043006903194">🎁</e> Бонус
# ============================================================
# РЕКЛАМНЫЕ ССЫЛКИ: СООБЩЕНИЯ (ОТСУТСТВУЮТ В UPSTREAM 0.8.0)
# ============================================================

msg-advertising-list =
    🎯 <b>Рекламные ссылки</b>

    { $count ->
    [0] Ссылок пока нет. Создайте первую.
    *[other] Всего ссылок: <b>{ $count }</b>
    }

msg-advertising-view =
    🎯 <b>{ $name }</b>  { $is_active ->
    [1] ✅ Активна
    *[0] ❌ Неактивна
    }
    Код: <code>{ $code }</code>
    Ссылка: <code>{ $deep_link }</code>
    { $is_partner_link ->
    [true] Выплаты партнёру: <b>{ $owner_name }</b>
    *[false] Без партнёра — выплат нет
    }

    <b>Бонусы при переходе:</b>
    • Баллы: <b>{ $bonus_points }</b>
    • Дней: <b>{ $bonus_days }</b>
    • Скидка навсегда: <b>{ $bonus_discount_pct }%</b>
    • Скидка на первую покупку: <b>{ $bonus_discount_once_pct }%</b>

    <b>Аналитика:</b>
    • Переходов: <b>{ $clicks_count }</b>
    • Уник. пользователей: <b>{ $unique_clicks }</b>
    • Получили бонус: <b>{ $bonus_issued_count }</b>
    • Активировали пробный: <b>{ $trial_count }</b>
    • Оплатили: <b>{ $paid_count }</b>
    • Выручка: <b>{ $revenue_rub } ₽</b>

msg-advertising-create-name =
    ✏️ Введите название ссылки (для вашего удобства):

msg-advertising-create-code =
    🔗 Введите код ссылки (только буквы и цифры, без пробелов):

    <i>Название: { $create_name }</i>

msg-advertising-edit-name =
    ✏️ Введите новое название:

msg-advertising-edit-bonus-points =
    🎯 Введите количество баллов (0 — без бонуса):

msg-advertising-edit-bonus-days =
    📅 Введите количество дней (0 — без бонуса):

msg-advertising-edit-code =
    🔗 <b>Новый код кампании</b>

    Сейчас: <code>{ $code }</code>
    Ссылка: <code>{ $deep_link }</code>

    Латиница и цифры, до 61 символа.

    <b>⚠️ Уже размещённые ссылки со старым кодом перестанут работать.</b> Статистика, начисления и привязка людей переедут вместе с кампанией — они висят не на коде.

msg-advertising-link-owner =
    🤝 <b>Кому засчитывать кампанию «{ $link_name }»</b>

    { $is_empty ->
    [true] <i>Партнёров пока нет — заведите их в разделе «Партнёры».</i>
    *[false] Выберите человека из списка. { $has_owner ->
    [true] Текущий отмечен галочкой — нажмите на него, чтобы снять привязку.
    *[false] { "" }
    }
    }

    <i>С оплат по этой кампании пойдут начисления выбранному партнёру. Уже начисленное не пересчитывается.</i>

msg-advertising-edit-bonus-discount =
    💸 Введите размер <b>постоянной</b> скидки, % от 0 до 100 (0 — без скидки).

    <i>Останется у человека навсегда и будет применяться ко всем продлениям.</i>

msg-advertising-edit-bonus-discount-once =
    🎟 Введите размер скидки <b>на первую покупку</b>, % от 0 до 100 (0 — без скидки).

    <i>Сгорает после первой оплаты. Пробный период её не тратит.</i>

msg-advertising-confirm-delete =
    ⚠️ Удалить ссылку <b>{ $delete_name }</b>?

    Вся аналитика будет потеряна.

msg-advertising-promo =
    ✉️ <b>Promo-сообщение</b> — { $name }
    Отправить: введите <code>@vpnrain_bot { $inline_query }</code> в любом чате

    <b>Фото:</b> { $promo_has_photo ->
    [1] ✅ Загружено
    *[0] ❌ Нет
    }
    <b>Текст:</b>
    { $promo_text_preview }

    <b>Кнопки:</b>
    { $promo_buttons_info }

msg-advertising-promo-edit-text =
    ✏️ Введите текст promo-сообщения (поддерживается HTML):

msg-advertising-promo-edit-markdown =
    📐 Пришлите текст поста разметкой Markdown.

    Так доступны заголовки, списки и чек-листы, таблицы, цитаты, сворачиваемые блоки, сноски и формулы — всего этого в обычном сообщении телеграма нет.

    <b>Коротко:</b> <code>**жирный**</code>, <code>*курсив*</code>, <code>~~зачёркнутый~~</code>, <code>||спойлер||</code>, <code>`код`</code>, <code>[текст](ссылка)</code>. Заголовок — решётка в начале строки, пункт списка — дефис, цитата — угловая скобка.

    <i>Вложение и кнопки добавляются отдельно, как обычно.</i>

msg-advertising-promo-edit-photo =
    🖼 Отправьте вложение для promo-сообщения: фото, видео или гифку.

    { $promo_has_photo ->
    [1] <i>Текущее вложение будет заменено.</i>
    *[0] { "" }
    }

msg-advertising-promo-send-confirm =
    🚀 <b>Отправить пост</b>

    Адресат: <b>{ $target_title }</b>

    { $is_channel ->
    [1] <i>Это канал. Пост уйдёт пересылкой через вашу личку с ботом — иначе телеграм срезает премиум-эмодзи. В шапке появится «Переслано».</i>
    *[0] { "" }
    }

msg-advertising-promo-send-target =
    🚀 <b>Отправить ботом</b>

    Перешлите сюда любое сообщение из канала — или пришлите его @имя либо id.

    <i>Бот должен быть админом канала с правом публикации. Отправка идёт от его имени, поэтому премиум-эмодзи и разметка сохраняются.</i>

msg-advertising-promo-button-label =
    ✏️ Введите текст кнопки (до 100 символов):

    <i>Можно использовать эмодзи, например: 🟢 Попробовать</i>

msg-advertising-promo-button-url =
    🔗 Введите URL для кнопки <b>{ $new_btn_label }</b>:

    <i>Должен начинаться с https://
    Или нажмите кнопку ниже, чтобы использовать ссылку бота.</i>

msg-advertising-promo-button-style =
    🎨 Выберите цвет кнопки <b>{ $new_btn_label }</b>:

msg-advertising-analytics =
    📊 <b>Аналитика — { $name }</b>

    { $period_days ->
    [0] Период: <b>Всё время</b>
    *[other] Период: <b>{ $period_days } дней</b>
    }

    • Уник. пользователей: <b>{ $unique_clicks }</b>
    • Получили бонус: <b>{ $bonus_issued_count }</b>
    • Активировали пробный: <b>{ $trial_count }</b>
    • Оплатили: <b>{ $paid_count }</b>
    • Выручка: <b>{ $revenue_rub } ₽</b>
    • Конверсия: <b>{ $conversion_pct }%</b>

msg-advertising-comparison =
    📊 <b>Сравнение кампаний</b>  ({ $count } ссылок)

    { $comparison_text }

# ============================================================
# РЕФЕРАЛЬНАЯ СИСТЕМА: СОБЫТИЯ
# Переопределён полностью, чтобы добавить атрибут .milestone
# ============================================================

event-referral =
    .attached =
    <b><e id="5431673473703389665">🎉</e> Вы пригласили друга!</b>

    <blockquote>
    Пользователь <b>{ $name }</b> присоединился по вашей пригласительной ссылке! Чтобы получить награду, убедитесь, что он совершит покупку подписки.
    </blockquote>

    .reward =
    <b><e id="5433718990532814817">💰</e> Вам начислена награда!</b>

    <blockquote>
    Пользователь <b>{ $name }</b> совершил платеж. Вы получили { $reward_type ->
    [POINTS] <b>{ $value } { $value ->
        [one] балл
        [few] балла
        *[more] баллов
        }</b>

    <i>Для использования баллов перейдите в раздел "Пригласить" в боте, чтобы узнать о доступных наградах и способах их использования.</i>
    [EXTRA_DAYS] <b>{ $value } доп. { $value ->
        [one] день
        [few] дня
        *[more] дней
        }</b> к вашей подписке!
    *[OTHER] <b>{ $value } { $reward_type }</b>
    }
    </blockquote>

    .reward-failed =
    <b><e id="5433773811495378237">❌</e> Не получилось выдать награду!</b>

    <blockquote>
    Пользователь <b>{ $name }</b> совершил платеж, но мы не смогли начислить вам вознаграждение из-за того что <b>у вас нет купленной подписки</b>, к которой можно было бы добавить { $value } { $reward_type ->
    [POINTS] { $value ->
        [one] балл
        [few] балла
        *[more] баллов
        }
    [EXTRA_DAYS] доп. { $value ->
        [one] день
        [few] дня
        *[more] дней
        }
    *[OTHER] { $reward_type }
    }.

    <i>Купите подписку, чтобы получать бонусы за приглашенных друзей!</i>
    </blockquote>

    .milestone =
    <b><e id="5431658449907789518">🏆</e> Новый уровень реферальной программы!</b>

    <blockquote>
    Вы достигли <b>{ $tier } уровня</b>!
    Оплативших рефералов: <b>{ $paid_referrals_count }</b>
    Персональная скидка: <b>{ $discount }%</b>
    </blockquote>

# Уведомления рекламных ссылок
ntf-ad =
    .publish-ok = ✅ <i>Пост отправлен.</i>
    .publish-failed = ⚠️ <i>Не удалось отправить: { $reason }</i>
    .bonus-received =
        <e id="5431744001361352063">🎁</e> <b>Вам начислен бонус за переход по рекламной ссылке!</b>
        { $bonus_points ->
        [0] { "" }
        *[other]
         • +{ $bonus_points } баллов
        }{ $bonus_days ->
        [0] { "" }
        *[other]
         • +{ $bonus_days } дней к подписке
        }{ $bonus_discount_pct ->
        [0] { "" }
        *[other]
         • Постоянная скидка { $bonus_discount_pct }%
        }{ $bonus_discount_once_pct ->
        [0] { "" }
        *[other]
         • Скидка { $bonus_discount_once_pct }% на первую покупку
        }

    .code-invalid = <e id="5431713747611722012">⚠️</e> <i>Рекламная ссылка не найдена или недоступна.</i>

btn-goto =
    .subscription = <e id="5433625699548179370">💳</e> Купить подписку
    .subscription-renew = <e id="5431450139698962745">🔄</e> Продлить подписку
    .contact-support = <e id="5433790699306787710">📩</e> Перейти в поддержку
    .main-menu = <e id="5433682723828966023">🏠</e> В главное меню

ntf-reactivation =
    .trial-unused-first =
    <e id="5431744001361352063">🎁</e> <b>Вам доступен бесплатный пробный период.</b>

    Он ещё не активирован. Включите его в главном меню и пользуйтесь сервисом бесплатно!

    .trial-unused-last =
    <e id="5431744001361352063">🎁</e> <b>Ваш пробный период всё ещё не активирован.</b>

    Это последнее напоминание. Активировать его можно в любой момент в главном меню.

    .trial-expired-offer =
    <e id="5431503878329774916">🎟</e> <b>Для вас действует скидка { $discount }% на подписку.</b>

    Ваш бесплатный пробный период закончился. Оформите подписку со скидкой — она действует { $days } дня!

    .trial-expired-last =
    <e id="5431537503628730769">⛔</e> <b>Внимание! Скидка { $discount }% заканчивается завтра.</b>

    После этого подписка будет доступна по обычной цене. Успейте оформить!

    .paid-expired-offer =
    <e id="5431503878329774916">🎟</e> <b>Для вас действует скидка { $discount }% на продление.</b>

    Ваша подписка закончилась, доступ к VPN приостановлен. Продлите её со скидкой — она действует { $days } дня!

    Если во время пользования сервисом возникли сложности, напишите в поддержку — поможем.

    .paid-expired-last =
    <e id="5431537503628730769">⛔</e> <b>Внимание! Скидка { $discount }% заканчивается завтра.</b>

    После этого продление будет доступно по обычной цене. Успейте продлить!

# ============================================================
# ПОДПИСИ РОЛЕЙ
# ------------------------------------------------------------
# Ветки выбираются по числовому значению роли из Role, а порядок членов там
# менялся: PARTNER уехал вниз, добавился ADVERTISER. Тот же блок лежит в
# assets/translations/ru/utils.ftl, но базовые переводы вшиты в образ, а он
# пока не пересобран — до пересборки правда живёт здесь.
# Метки перечислены целиком: слой custom.ftl отдельный, и сослаться на
# определения из базового набора отсюда нельзя.
# ============================================================

role-owner = Владелец
role-dev = Разработчик
role-admin = Администратор
role-advertiser = Рекламный менеджер
role-preview = Наблюдатель
role-user = Пользователь
role-partner = Партнёр

role = 
    { $role ->
    [7] { role-owner }
    [6] { role-dev }
    [5] { role-admin }
    [4] { role-preview }
    [3] { role-advertiser }
    [1] { role-partner }
    *[2] { role-user }
}

# ============================================================
# СМЕНА ТАРИФА: ЧТО ТЕРЯЕТСЯ
# ------------------------------------------------------------
# Смена не продлевает, а заменяет: старая подписка помечается удалённой,
# у новой срок считается с сегодня, трафик обнуляется. В базовом тексте
# про это сказано глухо — «без пересчёта оставшегося срока», — и человек
# узнаёт цену решения уже после оплаты.
#
# Отдельным сообщением, а не правкой msg-subscription-confirm, потому что
# тот ссылается на msg-subscription-details, а он на frg-payment-amount и
# дальше. Слой custom.ftl компилируется отдельным бандлом и до базовых
# определений не дотягивается, так что переопределение утащило бы сюда
# половину соседних сообщений — и три экрана оплаты вместе с ними.
#
# Ветка выбирается готовой строкой из питона: селекторы сопоставляют
# ключи как строки, и булево значение в ветку [true] не попадает.
#
# Первая строка варианта пустая нарочно: окно склеивает тексты через один
# перевод строки, и без неё предупреждение слипалось с базовым. Значок
# другой по той же причине — два одинаковых треугольника подряд читались
# как одна мысль, а это две разные: механика и её цена.
# ============================================================

msg-subscription-change-warning =
    { $change_warning ->
    [SHOW]
    { "" }
    <i><e id="5433773811495378237">❌</e> Оставшиеся <b>{ $change_days_lost }</b> и накопленный трафик <u>сгорят</u> — срок начнётся заново.</i>
    *[HIDE] { "!empty!" }
    }

# ============================================================
# ДОКУПКА УСТРОЙСТВ
# ------------------------------------------------------------
# Количество устройств перестало быть свойством тарифа: его можно
# докупить к действующей подписке, не трогая ни срок, ни трафик.
#
# Ветки везде выбираются готовой строкой из питона: селекторы
# сопоставляют ключи как строки, и булево значение туда не попадает.
#
# Сообщения самодостаточны и ни на что из базового набора не ссылаются —
# слой custom.ftl компилируется отдельным бандлом и до базовых
# определений не дотягивается.
# ============================================================

msg-device-addon =
    <b><e id="5433608163196706522">📱</e> Добавление устройства</b>

    { $addon_state ->
    [CEILING]
    На одной подписке мы даём до { $max_devices } устройств.

    <i>Нужно больше — напишите в поддержку, подберём корпоративные условия.</i>
    [DISABLED]
    <i><e id="5431713747611722012">⚠️</e> Докупка устройств сейчас недоступна. Попробуйте позже.</i>
    [NO_METHODS]
    <i><e id="5431713747611722012">⚠️</e> Сейчас нет доступного способа оплаты. Попробуйте позже.</i>
    [NOTHING_TO_PAY]
    <i><e id="5431713747611722012">⚠️</e> Подписка заканчивается сегодня — докупать устройство уже не к чему. Продлите её, и устройство можно будет выбрать сразу.</i>
    *[OK]
    <blockquote>
    • <b>Станет устройств</b>: { $new_total } вместо { $current_limit }
    • <b>Действует до</b>: { $expire_at }, как и подписка
    • <b>Оплата</b>: за оставшиеся { $days_left_text }
    </blockquote>

    <i>При продлении это устройство добавит { $monthly_rub } ₽ в месяц, все докупленные — { $monthly_total_rub } ₽.</i>

    Выберите способ оплаты:
    }

msg-device-addon-confirm =
    <b><e id="5433608163196706522">📱</e> Оплата устройства</b>

    <blockquote>
    • <b>Станет устройств</b>: { $new_total }
    • <b>К оплате</b>: { $amount }{ $currency }
    </blockquote>

    <i>После оплаты лимит поднимется сам. Срок подписки и трафик не изменятся.</i>

btn-device-addon =
    .add = ➕ Добавить устройство
    .pay-with = 💳 { $gateway_title } — { $amount }{ $currency }
    .pay = 💳 Оплатить { $amount }{ $currency }
    .cancel = ❌ Отмена

# Строка на экране устройств. Отдельным сообщением, а не правкой
# msg-menu-devices: тот ссылается на unlimited, empty и space из базового
# набора, и переопределение утащило бы их все сюда.
msg-menu-devices-addon =
    { $addon_hint ->
    [FULL]
    { "" }
    <e id="5431350621011746205">⚡</e> Свободных слотов нет. Ещё одно устройство — <b>{ $addon_now_price } ₽</b> до конца подписки, дальше { $next_device_price } ₽ в месяц.
    [FREE]
    { "" }
    Нужно больше устройств? Ещё одно — <b>{ $addon_now_price } ₽</b> до конца подписки, дальше { $next_device_price } ₽ в месяц.
    [CEILING]
    { "" }
    Это наш потолок для одной подписки. Нужно больше — напишите в поддержку, подберём корпоративные условия.
    *[HIDE] { "!empty!" }
    }

# ============================================================
# ОПИСАНИЕ СЧЁТА
# ------------------------------------------------------------
# У докупки свой вид покупки, которого в базовом наборе нет: там ветка по
# умолчанию печатает само значение, и в банке человек увидел бы «DEVICES
# подписки Solo». Обе метки перечислены целиком — сослаться на базовые
# определения отсюда нельзя.
#
# Ветка для подписки должна остаться слово в слово как в базовом наборе:
# это описание видят во всех платежах, а не только в докупке.
# ============================================================

purchase-type = { $purchase_type ->
    [NEW] Покупка
    [RENEW] Продление
    [CHANGE] Изменение
    [DEVICES] Устройство
    *[OTHER] { $purchase_type }
}

payment-invoice-description = { $purchase_type ->
    [DEVICES] Дополнительное устройство к подписке { $name } на { $duration }
    *[OTHER] { purchase-type } подписки { $name } на { $duration }
}

# ============================================================
# СОБЫТИЕ: ДОКУПЛЕНО УСТРОЙСТВО
# ------------------------------------------------------------
# Уходит и человеку в личку, и в тему событий — одним сообщением, как и
# остальные покупки.
#
# Не веткой event-subscription: тот собран из заголовков и фрагментов
# базового набора, а слой custom.ftl компилируется отдельным бандлом и до
# них не дотягивается — пришлось бы перенести сюда половину соседних
# определений.
#
# Поля берутся только простые. 'plan_device_limit' рядом лежит в форме
# для переводов, и подставить его значило бы напечатать кортеж — ровно
# то, что однажды уехало в чек налоговой.
#
# Телеграмный id печатается через NUMBER с выключенной группировкой:
# иначе Fluent разбивает его разрядами и получается '1 075 319 630'.
# ============================================================

event-devices-added =
    #DeviceAddonEvent

    <b><e id="5433608163196706522">📱</e> Событие: Докуплено устройство!</b>

    <blockquote>
    • <b>Пользователь</b>: { $name }
    • <b>ID</b>: <code>{ NUMBER($telegram_id, useGrouping: 0) }</code>
    • <b>Стало устройств</b>: { $devices_total }
    • <b>Оплачено</b>: { $final_amount }{ $currency }
    • <b>Способ</b>: { $gateway_type }
    </blockquote>

# ============================================================
# ЭКРАН УСТРОЙСТВ
# ------------------------------------------------------------
# Переопределён ради одного пробела: в базовом наборе строка про нехватку
# лимита приклеена к предыдущему блоку без разделителя, и у того, кто
# ничего не подключил, получалось «Подключено: 0 / 2Если не хватает...».
#
# Вместе с сообщением пришлось перенести сюда три метки, на которые оно
# ссылается: слой custom.ftl компилируется отдельным бандлом и до
# базовых определений не дотягивается. Определения те же, что в базовом
# наборе, слово в слово.
# ============================================================

space = {" "}
empty = { "!empty!" }
unlimited = ∞

msg-menu-devices =
    <b><e id="5433608163196706522">📱</e> Управление устройствами</b>

    Подключено: <b>{ $current_count } / { $max_count ->
    [0] { unlimited }
    *[LIMIT] { $max_count }
    }</b>

    { $has_devices ->
    [0] { empty }
    *[HAS] { $device_single_enabled ->
        [0] Для отвязки устройства обратитесь в техподдержку.
        *[OTHER] Нажмите на устройство чтобы удалить его.
        }
    }
