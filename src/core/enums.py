...
class Role(IntEnum):
    """
    Порядок задаёт старшинство.

    В базе роль лежит перечнем Postgres по имени, поэтому переставлять членов
    безопасно для данных. Но на числовых значениях завязаны две вещи:
    сравнения ролей между собой и селектор подписей 'role' в переводах —
    правя порядок здесь, надо править и его.

    PARTNER стоит ниже USER намеренно. Это аффилиат: обычный клиент, который
    получает долю с оплат, а не сотрудник. Пока он стоял в конце, он
    оказывался старше владельца, и is_privileged давал ему True — обход
    ограниченного режима и проверок правил.
    """

    PARTNER = auto()
    USER = auto()
    # Ведёт рекламные размещения: раздел рекламы целиком, но не админ.
    ADVERTISER = auto()
    PREVIEW = auto()
    ADMIN = auto()
    DEV = auto()
