...
from src.application.common import BotService, Notifier, Redirect, Remnawave, TranslatorRunner
from src.application.common.dao import NodeQuotaDao, PlanDao, SubscriptionDao, TransactionDao, UserDao
...
async def on_node_quota_test_notify(
    ...
    target_telegram_id: int = dialog_manager.dialog_data.get(TARGET_TELEGRAM_ID, 0)
    if not target_telegram_id:
        await callback.answer("❌ Telegram ID недоступен")
        return
...
async def on_node_quota_reset(
    ...
    from src.application.dto import UserDto as _UserDto
    user: _UserDto = dialog_manager.middleware_data[USER_KEY]
    target_telegram_id: int = dialog_manager.dialog_data.get(TARGET_TELEGRAM_ID, 0)
    if not target_telegram_id:
        await callback.answer("❌ Telegram ID недоступен")
        return
