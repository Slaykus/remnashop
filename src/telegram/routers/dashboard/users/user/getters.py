...
async def node_quota_getter(
    ...
    **kwargs: Any,
    ...
    quota = await node_quota_dao.get_by_telegram_id(target_telegram_id) if target_telegram_id else None
    ...
    period_start = quota.period_start.strftime(DATETIME_VIEW_FORMAT) if quota else "—"
    restricted_at = (
        quota.restricted_at.strftime(DATETIME_VIEW_FORMAT)
